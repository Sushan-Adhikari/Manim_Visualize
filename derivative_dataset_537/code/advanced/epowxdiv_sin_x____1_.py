...
class DerivativeVisualization(Scene):
    def construct(self):
        ...
        calc_step3 = MathTex(r"f'(x) = \frac{e^x(\sin x+1) - e^x\cos x}{(\sin x+1)^2}", font_size=22).next_to(calc_step2, DOWN, buff=0.25, aligned_edge=LEFT)
        calc_final = MathTex(r"f'(x) = \frac{e^x(\sin x+1-\cos x)}{(\sin x+1)^2}", font_size=26, color=YELLOW).next_to(calc_step3, DOWN, buff=0.3, aligned_edge=LEFT)
