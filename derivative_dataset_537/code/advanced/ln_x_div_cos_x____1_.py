...
class DerivativeVisualization(Scene):
    def construct(self):
        ...
        # Plot the function
        graph = axes.plot(f, color=BLUE, x_range=[0.1, 3.0]) # Adjusted x_range to avoid asymptote at pi
        ...
        calc_step3 = MathTex(r"f'(x) = \frac{(\cos x+1)/x + \ln x \sin x}{(\cos x+1)^2}", font_size=22).next_to(calc_step2, DOWN, buff=0.25, aligned_edge=LEFT)
        ...
        tangent = always_redraw(lambda: axes.plot(
            ...
            x_range=[x_tracker.get_value()-0.5, x_tracker.get_value()+0.5], # Smaller range for tangent
            color=GREEN
        ))
