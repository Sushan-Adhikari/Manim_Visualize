...
class DerivativeVisualization(Scene):
    def construct(self):
        ...
        # Define the function
        def f(x):
            return np.log(np.sqrt(x) + 33)
        
        # Plot the function
        graph = axes.plot(f, color=BLUE, x_range=[0.01, 3.99])
        func_label = MathTex(r"f(x) = \ln(\sqrt{x} + 33)", color=BLUE, font_size=36).to_corner(UL).shift(DOWN*0.3)
        ...
        calc_step1 = MathTex(r"f(x) = \ln(\sqrt{x} + 33)", font_size=22).next_to(calc_title, DOWN, buff=0.3, aligned_edge=LEFT)
        calc_step2 = MathTex(r"\text{Chain Rule: } \frac{d}{dx}[\ln(g)] = \frac{g'}{g}", font_size=22).next_to(calc_step1, DOWN, buff=0.25, aligned_edge=LEFT)
        calc_step3 = MathTex(r"f'(x) = \frac{1/(2\sqrt{x})}{\sqrt{x} + 33}", font_size=22).next_to(calc_step2, DOWN, buff=0.25, aligned_edge=LEFT)
        calc_final = MathTex(r"f'(x) = \frac{1}{2\sqrt{x}(\sqrt{x} + 33)}", font_size=26, color=YELLOW).next_to(calc_step3, DOWN, buff=0.3, aligned_edge=LEFT)
        ...
        def f_prime(x):
            return 1 / (2 * np.sqrt(x) * (np.sqrt(x) + 33))
        ...
        tangent = always_redraw(lambda: axes.plot(
            ...
            x_range=[max(0.01, x_tracker.get_value()-1.5), min(3.99, x_tracker.get_value()+1.5)],
            color=GREEN
        ))
